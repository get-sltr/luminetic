...
export const handler = async () => {
  ...
  // Find stuck scans — status is pending/analyzing/reconciling AND updatedAt is old
    ...
    try {
      ...
      if (scan.creditCharged === true && scan.creditRefunded !== true) {
          ...
          TransactItems: [
            {
              Update: {
                ...
                ConditionExpression: "#s = :currentStatus AND creditCharged = :true AND (attribute_not_exists(creditRefunded) OR creditRefunded = :false)",
                ExpressionAttributeNames: { "#s": "status" },
                ExpressionAttributeValues: {
                  ":error": "error",
        ...
        await db.send(new UpdateCommand({
          ...
          ExpressionAttributeValues: {
            ":error": "error",
            ":msg": scan.creditRefunded === true
              ? "Analysis timed out. Your credit has been preserved — please try again."
