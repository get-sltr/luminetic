...
export async function POST(request: NextRequest) {
  ...
  let scanSKForRefund: string | null = null;
  ...
  try {
    ...
    if (guardedText) {
      const guard = await guardInput(guardedText, "prompt-injection");
      if (guard.blocked) {
    ...
    await db.send(new PutCommand({
      ...
      Item: {
        ...
        scanChargeType: scanCreditCharged ? "paid" : isFreeScan ? "free" : "none",
    ...
    if (scanRecordCreated && chargedUserId && scanSKForRefund) {
      try {
        await db.send(new UpdateCommand({
          TableName: TABLE,
          Key: { PK: `USER#${chargedUserId}`, SK: scanSKForRefund },
          UpdateExpression: "SET #s = :error, errorMessage = :msg, updatedAt = :now",
          ExpressionAttributeNames: { "#s": "status" },
          ExpressionAttributeValues: {
            ":error": "error",
            ":msg": "Analysis failed to start. Your credit has been restored — please try again.",
            ":now": new Date().toISOString(),
          },
        }));
      } catch (statusErr) {
        console.warn("[analyze-stream] Failed to mark scan as error:", statusErr);
      }
    }
    if (scanCreditCharged && chargedUserId) {
      try {
        if (scanRecordCreated && scanSKForRefund) {
          await refundChargedScanCredit(chargedUserId, scanSKForRefund);
        } else {
          await refundScanCredit(chargedUserId);
        }
      } catch (refundErr) {
        console.warn("[analyze-stream] Credit refund failed:", refundErr);
      }
    }
